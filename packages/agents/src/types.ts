...
export const EMPLOYER_ADVOCATE_REASON_CODES = [
  "missing_required_ref",
  "invalid_frozen_ref",
  "unauthorized_manifest_ref",
  "unfiltered_counterparty_data",
  ...
  "scoring_dimensions_invalid",
  "human_input_pause_refused",
  "rubric_bias_gate_missing",
  "protected_class_boundary_refused",
  "decision_content_refused",
  "audit_unavailable",
] as const;
export type EmployerAdvocateReasonCode = (typeof EMPLOYER_ADVOCATE_REASON_CODES)[number];

export type EmployerAdvocateOperation = "turn" | "score";

export interface EmployerAdvocateFrozenRefs {
  readonly contract_ref: ContractRef;
  readonly prompt_ref: PromptRef;
  readonly model_ref: ModelRef;
  readonly manifest_ref: ManifestRef;
...
export interface EmployerAdvocatePrincipalView {
  readonly employer_ticket_id: string;
  readonly role_summary: string;
  readonly requirements: readonly string[];
  readonly constraints: readonly string[];
  readonly threshold: number;
}

export interface EmployerAdvocateRuntime {
  readonly repository: AiRepository;
  readonly gateway: GatewayAdapter;
  readonly caller: ScopedAiPrincipal;
  readonly caller_scope: string;
...
export interface EmployerAdvocateRunInput {
  readonly run_id: string;
  readonly match_ticket_id: string;
  readonly operation: EmployerAdvocateOperation;
  readonly round?: number;
  readonly refs: EmployerAdvocateFrozenRefs;
  readonly principal_view: EmployerAdvocatePrincipalView;
  readonly counterparty_projection: FilteredCounterpartyProjection;
  readonly context: NegotiationContext;
  readonly allowed_tool_names: readonly string[];
...
export interface EmployerAdvocateTurnResult {
  readonly run_id: string;
  readonly side: "employer";
  readonly round: number;
  readonly message_to_counterparty: string;
  readonly internal_rationale: string;
  readonly done_signal: boolean;
  readonly flag_proposals: readonly string[];
  readonly invocation_ref?: string;
  readonly frozen_refs: EmployerAdvocateFrozenRefs;
  readonly audit_refs: readonly string[];
}

export interface EmployerDimensionScore {
  readonly dimension_id: string;
  readonly score: number;
  readonly rationale: string;
  readonly evidence_refs?: readonly string[];
...
export interface EmployerAdvocateScoringResult {
  readonly run_id: string;
  readonly side: "employer";
  readonly rubric_ref: RubricRef;
  readonly dimension_scores: readonly EmployerDimensionScore[];
  readonly headline_rationale: string;
  readonly flag_proposals: readonly string[];
  readonly ignored_holistic_score: number | null;
  readonly rejected_decision_content: readonly string[];
  readonly protected_class_boundary?: string;
  readonly invocation_ref?: string;
  readonly frozen_refs: EmployerAdvocateFrozenRefs;
  readonly audit_refs: readonly string[];
}

export interface EmployerAdvocateRefusal {
  readonly run_id: string;
  readonly side: "employer";
  readonly operation: EmployerAdvocateOperation;
  readonly reason_code: EmployerAdvocateReasonCode;
  readonly message: string;
  readonly affected_refs?: Record<string, unknown>;
  readonly audit_refs: readonly string[];
  readonly created_at: string;
...
export type EmployerTurnDecision =
  | { readonly ok: true; readonly result: EmployerAdvocateTurnResult }
  | { readonly ok: false; readonly refusal: EmployerAdvocateRefusal };

export type EmployerScoringDecision =
  | { readonly ok: true; readonly result: EmployerAdvocateScoringResult }
  | { readonly ok: false; readonly refusal: EmployerAdvocateRefusal };

export interface EmployerAdvocateScoreDraft {
  readonly dimension_scores: readonly EmployerDimensionScore[];
  readonly headline_rationale: string;
  readonly flag_proposals: readonly string[];
  readonly model_holistic_score?: number;
  readonly decision_content?: readonly string[];
  readonly protected_class_boundary?: string;
}

export interface EmployerAdvocateScoringInput extends EmployerAdvocateRunInput {
  readonly rubric: RubricVersion;
  readonly score_draft?: EmployerAdvocateScoreDraft;
}

export interface EmployerAdvocateEvalCase {
  readonly eval_case_id: string;
  readonly category:
    | "strong_match"
    | "weak_match"
