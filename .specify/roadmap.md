...
| ID | Feature | Slug | Priority | Complexity | Constitution refs |
|----|---------|------|----------|------------|-------------------|
| F16 | Channel adapter framework + `ChannelMessage` interface | `16-channels-core` | P0 | M — Complete (merged to `main`, PR #53) | III.2 (semantic clarity for adapters); IV (SoC) |
...
| F20 | Conversational onboarding & seeker product flows | `20-seeker-conversational-flows` | P0 | XL — Complete (merged to `main`, PR #57) | I.4.1 (data minimization); III.1; PRD §6.1 |
...
| F22 | Employer admin console | `22-employer-admin-console` | P0 | L — Implemented (`022-employer-admin-console`; PR review) | III.1; III.4 (WCAG); I.5.1 (MFA for admin seats) |
...
### Stage 6 checklist
- [x] **F16** Channel adapter framework _(merged to `main`, PR #53; quickstart evidence recorded)_
...
- [x] **F20** Conversational onboarding & flows _(merged to `main`, PR #57; quickstart evidence recorded)_
...
- [ ] **F22** Employer admin console _(implemented on branch `022-employer-admin-console`; quickstart evidence and review artifacts recorded; PR review pending merge)_
...
1. **Review and merge F22 Employer admin console:** monitor the PR checks/mergeability, address review feedback if any, then merge and run the standard post-merge cleanup.
...
| 1.5.33  | 2026-05-25 | Status update for F22 start: Employer admin console is active on branch `022-employer-admin-console`, active Spec Kit pointers target `.specify/specs/022-employer-admin-console`, and specify/clarify/plan/tasks artifacts are initialized with `/speckit-analyze` as the next step. PATCH-style status amendment. |
| 1.5.32  | 2026-05-25 | Status update for F21 merge: Seeker web surface is complete and merged to `main` in PR #58, quickstart evidence is recorded, local/remote branch cleanup is complete, and Stage 7 next work is F22 Employer admin console. PATCH-style status amendment. |
| 1.5.31  | 2026-05-25 | Status update for F21 implementation: Seeker web surface is implemented on branch `021-seeker-web-surface`, quickstart evidence is recorded, code/security-accessibility reviews are added, and PR/merge follow-through remains pending. PATCH-style status amendment. |
| 1.5.30  | 2026-05-25 | Status update for F21 start: Seeker web surface is active on branch `021-seeker-web-surface`, active Spec Kit pointers target `.specify/specs/021-seeker-web-surface`, and specify/clarify/plan/tasks artifacts are initialized with `/speckit-analyze` as the next step. PATCH-style status amendment. |
| 1.5.29  | 2026-05-25 | Status update for F20 merge: Conversational onboarding & seeker product flows is complete and merged to `main` in PR #57, quickstart evidence is recorded, and next roadmap work is F21 Seeker web surface. PATCH-style status amendment. |
...
| 1.5.23  | 2026-05-23 | Status update for F18 merge: Email channel adapter is complete and merged to `main` in PR #55, quickstart evidence is recorded, local/remote branch cleanup is complete, and Stage 6 next work is F19 Web-chat channel adapter. PATCH-style status amendment. |
| 1.5.22  | 2026-05-23 | Status update for F18 implementation: Email channel adapter package slice implemented in `@spyglass/email-channel`, quickstart evidence recorded, analyze/code/security review artifacts added, and PR/merge follow-through remains pending. PATCH-style status amendment. |
| 1.5.21  | 2026-05-23 | Status update for F18 start: Email channel adapter is active on branch `018-email-channel`, active Spec Kit pointers target `.specify/specs/018-email-channel`, and specify/clarify/plan artifacts are initialized with Resend-first provider-parsed inbound and Spyglass reply-alias threading decisions recorded. PATCH-style status amendment. |
| 1.5.20  | 2026-05-23 | Status update for F17 merge: Telegram channel adapter is complete and merged to `main` in PR #54, quickstart evidence is recorded, and Stage 6 next work is F18 Email channel adapter. PATCH-style status amendment. |
| 1.5.19  | 2026-05-23 | Status update for F17 implementation: Telegram channel adapter package slice implemented in `@spyglass/telegram-channel`, quickstart evidence recorded, analyze/code/security review artifacts added, and PR/merge follow-through remains pending. PATCH-style status amendment. |
| 1.5.18  | 2026-05-23 | Status update for F17 planning: Telegram channel adapter clarify pass found no blocking ambiguity; plan, research, data model, contracts, quickstart, and 54-task implementation breakdown are initialized. Next step is `/speckit-analyze` before implementation. PATCH-style status amendment. |
| 1.5.17  | 2026-05-23 | Status update for F17 start: Telegram channel adapter is active on branch `017-telegram-channel`, active Spec Kit pointers target `.specify/specs/017-telegram-channel`, and the initial F17 specification and requirements checklist are initialized after reading PRD and Parley channel-surface guidance. PATCH-style status amendment. |
| 1.5.16  | 2026-05-23 | Status update for F16 merge: Channel adapter framework is complete and merged to `main` in PR #53, local/remote branch cleanup is complete, and Stage 6 next work is F17 Telegram channel adapter. PATCH-style status amendment. |
| 1.5.15  | 2026-05-22 | Status update for F16 implementation: Channel adapter framework package slice implemented in `@spyglass/channels-core`, quickstart evidence recorded, and analyze/code/security review artifacts added. PR/merge follow-through remains pending. PATCH-style status amendment. |
| 1.5.14  | 2026-05-22 | Status update for F16 start: Channel adapter framework is active on branch `016-channels-core`, active Spec Kit pointers target `.specify/specs/016-channels-core`, and spec/plan/tasks artifacts are initialized after reading PRD and Parley channel-surface guidance. PATCH-style status amendment. |
...
| 1.5.9   | 2026-05-22 | Status update for F14 task generation: Employer Advocate Agent tasks initialized for branch `014-employer-advocate`; next step is analysis remediation then implementation. PATCH-style status amendment. |
| 1.5.8   | 2026-05-22 | Status update for F14 planning: Employer Advocate Agent plan, research, data model, threat model, contracts, and quickstart artifacts initialized for branch `014-employer-advocate`. PATCH-style status amendment. |
| 1.5.7   | 2026-05-22 | Status update for F14 start: Employer Advocate Agent is active on branch `014-employer-advocate`, active Spec Kit pointers now target `.specify/specs/014-employer-advocate`, and the initial F14 spec is initialized. PATCH-style status amendment. |
