...
| PTH11 Neon `test_harness` schema persistence | In progress | Active Spec Kit slice: replace/local-augment `LocalFileProductResultStore` with a Neon-backed store targeting an isolated `test_harness` schema outside production. |
