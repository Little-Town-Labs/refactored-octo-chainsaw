...
| **Operationalization critical path** | PTH11 -> PTH12 -> PTH13 -> PTH14 -> PTH15 complete; PTH16 remains |
...
| PTH11 Neon `test_harness` schema persistence | Complete | Merged to `main` in PR #80; product harness now exposes a Neon/Postgres-backed result store for the isolated `test_harness` schema, schema/table/index initialization, JSONB snapshot persistence, indexed summary columns, idempotent duplicate writes, filtered read/list support, and local fake-SQL coverage. |
| PTH12 Durable artifact storage | Complete | Merged to `main` in PR #81; product harness now exposes provider-neutral artifact storage contracts, a deterministic local-file durable store, SHA-256 checksums, retention class and provider metadata, idempotent writes, conflict rejection, redaction and safe-metadata validation, and `RunArtifact` conversion for Neon result-store snapshots. |
...
| Feature | Exit Criteria |
|---------|---------------|
| PTH11 | `ProductResultStore` has a Neon-backed implementation that creates/uses an isolated `test_harness` schema, persists all PTH03-PTH10 snapshot categories, supports idempotent writes, and keeps production schemas untouched. Complete and merged to `main` in PR #80. |
| PTH12 | Large report, browser, trace, video, and transcript artifacts can be written to durable storage; result-store rows contain safe metadata refs, checksums, redaction status, and retention class. Complete and merged to `main` in PR #81. |
| PTH13 | Browser journeys can run through Browserbase-backed headless Playwright for preview/prod replay and canaries, with screenshots/traces/session refs captured as artifacts. Complete and merged to `main` in PR #82. |
...
1. **PTH16 Alpha harness operations runbook**: close the operational loop with provisioning, secret, retention, report-interpretation, and incident-response guidance.
2. **PTH17 Camofox browser evaluation spike**: optional follow-up only if Browserbase or stock Playwright exposes bot-detection friction on first-party Spyglass preview/prod surfaces.
